...
def saveArchive(backup_directory):
    ...
    if not os.path.exists(os.path.dirname(archive_file)):
        print
        'Creating directory: ' + os.path.dirname(archive_file)
...
def main():
    ...
    # Output .mov path
    mp4_output_path = houdini_dir + '/flip/' + hou.getenv('HIPNAME') + '/' + \
                      os.path.splitext(os.path.split(backup_file_path)[1])[0] + '.mp4'
    ...
    if not os.path.exists(os.path.dirname(flipbook_output_path)):
        print
        'Creating directory: ' + os.path.dirname(flipbook_output_path)
    ...
    # Set the output path
    print
    'Flipbook output path: ' + flipbook_output_path
